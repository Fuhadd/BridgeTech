...
class FailedConnectScreen extends StatefulWidget {
  const FailedConnectScreen({
    Key? key,
    required this.currentUser,
  }) : super(key: key);
  final AppUser currentUser;
  static const routeName = '/failedconnect';

  @override
  State<FailedConnectScreen> createState() => _FailedConnectScreenState();
}

class _FailedConnectScreenState extends State<FailedConnectScreen> {
  ...
  getChatRooms() async {
    chatRoomsStream = await FirestoreRepository()
        .getFailedConnections(widget.currentUser.id!);
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: MessageAppar(
            context, 'Failed Requests', widget.currentUser.imageUrl),
        body: chatRoomsList());
  }
}

...
class _SentRequestScreenState extends State<SentRequestScreen> {
    ...
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.hasData && snapshot.data?.docs.length != 0) {
          ...
          return NoUserWidget(
            mainText: "There are no pending invites",
            subText: "",
          );
        } else {
          return buildChatShimmer();
  ...
  getChatRooms() async {
    chatRoomsStream =
        await FirestoreRepository().getSentRequest(widget.currentUser.id!);
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar:
            MessageAppar(context, 'Sent Requests', widget.currentUser.imageUrl),
        body: chatRoomsList());
  }
}
